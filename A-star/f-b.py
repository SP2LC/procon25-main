...
def astar_step(queue, checked_nodes, table, min_distance,tag):
...
def forward(problem, answer, checked_nodes, result_queue):
  ...
  while len(queue) != 0:
    ...
    if queue[0][1].board == answer: #仮に取り出したキューが正答と一致したら終了
      print "forward goal"
    ...
    forward_min = min(forward_min, astar_step(queue, checked_nodes, distance_table, forward_min, my_tag))
...
def back(problem, answer, checked_nodes, result_queue):
  ...
  while len(queue) != 0:
    ...
    if kill_flag:
      ...
      return
    operations = queue[0][2]
    if queue[0][1].board == problem: #仮に取り出したキューが正答と一致したら終了
      print "back goal"
      print operations_to_list(operations)
      print "cost=%d" % caliculate_cost(operations)
      ALL_COST = caliculate_cost(operations)
    ...
    back_min = min(back_min, astar_step(queue, checked_nodes, distance_table, back_min, my_tag))

def solve(sortedImages, splitColumns, splitRows, limit, sel_rate, exc_rate):
    global LIMIT_SELECTION, SELECTON_RATE, EXCHANGE_RATE, distance_table, rows, columns, kill_flag
