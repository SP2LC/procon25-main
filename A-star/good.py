...
def point_md(point,board):
    ...
    ans = x + y
    return ans
def fast_distance_to_goal(looking_node,node):
    ...
    exchange_distance  = point_md(selection,parent_board) - point_md(exchange ,child_board)
    selection_distance = point_md(exchange ,parent_board) - point_md(selection,child_board)
...
def solve(sortedImages, splitColumns, splitRows, limit, sel_rate, exc_rate):
    global LIMIT_SELECTION, SELECTON_RATE, EXCHANGE_RATE, distance_table, ALL_COST
    ...
    for key, node in next_nodes.items() : #中身全部取り出すぜー
        ...
        if node.board != None :
            h_star = fast_distance_to_goal(Node(problem,(0,0),(0,0),static_h_star),node)
    ...
    while not queue.is_empty: #キューの長さ分くりかえすでー
        dummy, looking_node, operations, selection_count = queue.pop() #キューの先頭を取り出す
