...
def encode_answer_format(RLUD_text1,RLUD_text2,selection,first_selection, sel_count):
    answer_text = ""

    if selection == (-1,-1):
        selection_count = sel_count # 1
        answer_text  = str(selection_count) + "\r\n" + "%X%X"%(first_selection[1],first_selection[0]) +"\r\n"+ str(len(RLUD_text1)) +"\r\n"+ RLUD_text1 
    else:
        selection_count = sel_count + 1 #2
...
def solve(sortedImages, splitColumns, splitRows, limit, sel_rate, exc_rate):
    global LIMIT_SELECTION, SELECTON_RATE, EXCHANGE_RATE, distance_table,answer_text,ALL_COST, sel_count
    ...
    answer_text += "%X%X"%(static_first_selection_positon[1],static_first_selection_positon[0]) +"\r\n"+ "UNKNOWN" +"\r\n"
    ...
    for i, line in enumerate(lines):
      if line == "UNKNOWN":
        encode_text += str(len(lines[i+1])) + "\r\n"
      elif i + 1 == len(lines):
        encode_text += line
      else:
        encode_text += line + "\r\n"
    #encode_text = answer_text
    check_matrix(answer, problem, selection_positon)
    return encode_text
